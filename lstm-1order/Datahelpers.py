...
class Datahelper(object):
    def __init__(self, data_dir, limits):
        ...
        self.tag_1o_idx = self.get_mul_dict(tag_data)
        self.idx_1o_tag = {self.tag_1o_idx[tag]: tag for tag in self.tag_1o_idx}

        self.train_set = self.get_data(self.train_data_path, self.train_feat_path, self.word2idx, self.pos2idx, self.tag_1o_idx, self.feat2idx)
        self.dev_set = self.get_data(self.dev_data_path, self.dev_feat_path, self.word2idx, self.pos2idx,self.tag_1o_idx, self.feat2idx)
        self.test_set = self.get_data(self.test_data_path, self.test_feat_path, self.word2idx, self.pos2idx, self.tag_1o_idx, self.feat2idx)
    ...
    def get_data(self, path, feat_path, word_dict, pos_dict, tag_dict, feat_dict):
        ...
        tag_data = [[tag_dict[w] if w in tag_dict else tag_dict['UNKNOWN'] for w in sentence] for sentence in tag_data]
        ...
        return word_data, pos_data, tag_data, feat_data
